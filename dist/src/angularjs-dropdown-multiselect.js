...
directiveModule.directive('ngDropdownMultiselect', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
	return {
		...
		template: function(element, attrs) {
			...
			template += '<div class="dropdown-menu pb-0" ng-if="open" ng-class="{\'dropdown-menu-right\':settings.pullRight}" ng-style="{display: open ? \'block\' : \'none\', height : settings.scrollable ? settings.scrollableHeight : \'auto\', overflow: \'auto\' }" >';
            ...
            template += '<div class="dropdown-divider mb-0" ng-if="(settings.showCheckAll && settings.selectionLimit > 0) || settings.showUncheckAll || settings.showEnableSearchButton"></div>';
			template += '<div class="scrollable-list">';
			if (groups) {
				template += '<a href ng-click="option.disabled || setSelectedItem(getPropertyForObject(option,settings.idProp), false, true, $event)" ng-disabled="option.disabled" ng-keydown="option.disabled || keyDownLink($event)" role="menuitem" class="dropdown-item" ng-repeat-start="option in orderedItems | filter:getFilter(input.searchFilter)" ng-show="getPropertyForObject(option, settings.groupBy) !== getPropertyForObject(orderedItems[$index - 1], settings.groupBy)" role="presentation" class="dropdown-header">{{ getGroupLabel(getPropertyForObject(option, settings.groupBy)) }}</a>';
				template += '<a href ng-click="option.disabled || setSelectedItem(getPropertyForObject(option,settings.idProp), false, true, $event)" ng-disabled="option.disabled" ng-keydown="option.disabled || keyDownLink($event)" role="menuitem" class="dropdown-item" data-ng-class="{\'active\': isChecked(getPropertyForObject(option,settings.idProp)) && settings.styleActive, \'disabled\': option.disabled}" ng-repeat-end role="presentation">';
			} else {
				template += '<a href ng-click="option.disabled || setSelectedItem(getPropertyForObject(option,settings.idProp), false, true, $event)" ng-disabled="option.disabled" ng-keydown="option.disabled || keyDownLink($event)" role="menuitem" class="dropdown-item" data-ng-class="{\'active\': isChecked(getPropertyForObject(option,settings.idProp)) && settings.styleActive, \'disabled\': option.disabled}" role="presentation" ng-repeat="option in options | filter:getFilter(input.searchFilter)">';
			}

			template += '<span dm-dropdown-static-include="{{settings.template}}"></span></a>';
			template += '</a>';
			...
			template += '<div class="border-top py-2 px-3"><div class="text-center"><button class="btn btn-primary btn-block" ng-click="toggleDropdown()">Apply</button></div></div>';
