	...
	directiveModule.directive('ngDropdownMultiselect', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
		return {
			...
			template: function(element, attrs) {
				...
				if (groups) {
					template += '<h6 role="menuitem" class="dropdown-header" ng-repeat-start="option in (filteredOptions = (orderedItems | filter:getFilter(input.searchFilter)))" ng-show="getPropertyForObject(option, settings.groupBy) !== getPropertyForObject(filteredOptions[$index - 1], settings.groupBy)" role="presentation">{{ getGroupLabel(getPropertyForObject(option, settings.groupBy)) }}</h6>';
